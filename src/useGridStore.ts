        ...
        set((state) => {
          ...
          for (let i = 0; i < rows; i++) {
            const allZeros = matrix[i]
              .slice(0, columns - 1)
              .every((val) => val === 0); // التأكد من أن كل العناصر قبل العمود الأخير هي أصفار
            if (allZeros && matrix[i][columns - 1] !== 0) {
              // إذا كانت النتيجة النهائية غير صفرية
          ...
          if (inconsistentSystem) {
            return {
              solveText:
                "لا يوجد حل لأن النظام يحتوي على تناقض (صف يحتوي على أصفار ولكن النتيجة غير صفرية).<br>",
              solved: true,
            };
          }
          ...
          if (rows < columns - 1) {
            // إذا كان عدد الصفوف أقل من عدد المتغيرات
            return {
              solveText:
                "النظام غير صالح لأنه يحتوي على عدد معادلات أقل من عدد المتغيرات.<br>",
              solved: true,
            };
          }
          ...
          // نص لعرض الحل
          let text =
            "حل النظام باستخدام تحويل الصفوف (Row Echelon Form):<br><br>";
          ...
          for (let col = 0; col < columns - 1; col++) {
            ...
            while (pivotRow < rows && matrix[pivotRow][col] === 0) {
              // التحقق من وجود عنصر رئيسي غير صفري
              pivotRow++;
            }
            ...
            if (pivotRow !== leadingRow) {
              // التبديل بين الصفوف إذا لزم الأمر
              [matrix[leadingRow], matrix[pivotRow]] = [
                matrix[pivotRow],
                matrix[leadingRow],
              ];
              text += `تم تبديل الصف ${leadingRow + 1} مع الصف ${
                pivotRow + 1
              }<br><br>`;
            }
            ...
            if (leadElement !== 0) {
              ...
              text += `تم قسمة جميع عناصر الصف ${
                leadingRow + 1
              } على ${leadElement.toFixed(2)} لجعل العنصر الرئيسي في العمود ${
                col + 1
              } يساوي 1<br><br>`;
            }
            ...
            for (let i = leadingRow + 1; i < rows; i++) {
              ...
              if (factor !== 0) {
                text += `تم ضرب جميع عناصر الصف ${
                  leadingRow + 1
                } في ${factor.toFixed(2)} وطرحها من الصف ${
                  i + 1
                } لتصفير العنصر في العمود ${col + 1}<br><br>`;
              }
            }
          ...
          for (let i = 0; i < rows; i++) {
            const allZeros = matrix[i]
              .slice(0, columns - 1)
              .every((val) => val === 0); // التحقق من الصفوف التي تحتوي على أصفار
            if (allZeros && matrix[i][columns - 1] !== 0) {
              // التحقق إذا كانت النتيجة النهائية غير صفرية
              text +=
                "لا يوجد حل لأن هناك صفًا يحتوي على أصفار ولكن النتيجة غير صفرية.<br>";
